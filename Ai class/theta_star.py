            ...
            for node in self.successor(inspect_node.coordinates):
                if node not in self.visited:
                    ...
                    if self.lineofsight(inspect_node.root.coordinates, node.coordinates):
                        inspect_distance = inspect_node.root.g + self.distance(inspect_node.root.coordinates,
                                                                               node.coordinates)
    ...
    def successor(self, coordinate):
        ...
        for x in range(coordinate[0] - 1, coordinate[0] + 2):
            for y in range(coordinate[1] - 1, coordinate[1] + 2):
                if (x != coordinate[0] or y != coordinate[1]) and (
                        x <= x_max and x > x_min and y <= y_max and y > y_min) and self.reachable(coordinate, [x, y]):
        ...
        if dx >= dy:
            while x0 != x1:
                ...
                if f != 0 and [x0 + ((sx - 1) / 2), y0 + ((sy - 1) / 2)] in self.blocked:
                    return False
                if dy == 0 and [x0 + ((sx - 1) / 2), y0] in self.blocked and [x0 + ((sx - 1) / 2),
                                                                              y0 - 1] in self.blocked:
                    return False
                x0 = x0 + sx
        else:
            while y0 != y1:
                ...
                if f != 0 and [x0 + ((sx - 1) / 2), y0 + ((sy - 1) / 2)] in self.blocked:
                    return False
                if dx == 0 and [x0, y0 + ((sy - 1) / 2)] in self.blocked and [x0 - 1,
                                                                              y0 + ((sy - 1) / 2)] in self.blocked:
                    return False
                y0 = y0 + sy
